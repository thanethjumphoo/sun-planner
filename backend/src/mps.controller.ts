...
export class MpsController {
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      const isByproductSpec = (spec: ProductSpec): boolean => {
        ...
        return ids.some((id) => {
          ...
          return (
            node && (node.type === 'BY-PRODUCT' || node.type === 'CO-PRODUCT')
          );
        });
      };
      ...
      const sizeSupplyMap = new Map<
        string,
        { total: number; bins: Record<string, number> }
      >();
      ...
      for (const order of mainChillOrders) {
        ...
        for (
          ...
          const allocQty = Math.round(
            Math.min(availableQty, remainingQty, totalRmForDate),
          );
      ...
      const dailyByproductSupply = new Map<
        string,
        Record<string, { name: string; qty: number; processName: string }>
      >();
      ...
      for (const order of byprodChillOrders) {
        ...
        const spec = specMap.get(order.erpOrderItemCode);
        const bpId = spec?.masterYieldIds
          ?.split(',')
          .map((id: any) => id.trim())[0];
        ...
        if (bpId) {
          for (
            ...
            const bpSupply =
              daySupply && daySupply[bpId] ? daySupply[bpId].qty : 0;
      ...
      for (let i = 1; i <= daysInMonth; i++) {
        ...
        const cuttingStaff =
          demand > 0 ? (dailyStaff.get(dayStr) || 0) / 10 : 0;
  ...
  async exportPlan(@Param('id') id: number, @Res() res: express.Response) {
    ...
    orders.forEach((o) =>
      itemMap.set(o.itemCode, specMap.get(o.itemCode) || o.itemDesc),
    );
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=MPS_Plan_${plan.targetMonth}.xlsx`,
    );
