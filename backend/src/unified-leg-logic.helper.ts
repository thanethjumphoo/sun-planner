      ...
      if (isBil) {
         ...
         if (classification === 'BL_BLOCK') {
            ...
            if (remainingToProduce > 0 && availRmForDebone > 0) {
               const rmNeeded = remainingToProduce / productYield;
               ...
               const producedFromRm = Math.round(rmToUse * productYield);
            ...
            if (icutHoursRemaining > 0 && availRmForDebone > 0) {
               const rmNeeded = remainingQty / mainYieldPct;
               ...
               if (rmToUse > 0) {
                  ...
                  const produced = Math.round(rmToUse * mainYieldPct);
                  const blockProduced = rmToUse - produced;
            ...
            const rmNeeded = remainingQty / activeYield;
            const rmToUse = Math.min(availRmForDebone, rmNeeded);
            if (rmToUse > 0) {
               sup.totalLegRm -= rmToUse;
               capTracker.debonedKg += rmToUse;
               
               productProduced = Math.round(rmToUse * activeYield);
               generateByproducts(dateStr, rmToUse, spec);
