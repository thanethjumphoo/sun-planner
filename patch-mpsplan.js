...
const newBlock = `                          {/* Demand by Size Summary — expandable */}
                          {(() => {
                            const isBilPlan = currentPlan?.partType === 'bil' || currentPlan?.partType === 'bl' || currentPlan?.partType === 'leg';
                            if (!isBilPlan) {
                                // Fillet logic (original fallback)
                                // Not strictly required if we only fix BL/BIL, but let's keep the existing structure if needed.
                            }
                            ...
                            } else if (currentPlan?.partType === 'bl') {
                              return renderDemandBySize('Demand by RM Size (BL)', 'bl', mainOrders);
                            } else if (currentPlan?.partType === 'bil') {
                              return renderDemandBySize('Demand by RM Size (BIL)', 'bil', mainOrders);
                            } else {
                              return renderDemandBySize('Demand by RM Size', 'fillet', mainOrders);
                            }
                          })()}
